...
void testAlignedMemory() {
	memories_module(0,0,0);
	Test_begin("AlignedMemory", "Test start（64bits）...");
	...
	void* obj3 = AlignedMemory_alloc(mm,6, MemoryKind_normal);
	Test_assert("Memory.alloc"
		// 翻页，有第二页
		, mm->chunks[0]->page->next!=0 
		// 第二页没有后继页面
	...
	void* obj4 = AlignedMemory_alloc(mm, 8, MemoryKind_normal);
	Test_assert("Memory.re-alloc",obj4==obj3,"应该在被释放的内存上obj4==obj3");
	void* obj5 = AlignedMemory_alloc(mm, 8, MemoryKind_normal);
	Test_assert("Memory.re-alloc", obj5 == obj4-8, "内存是连续的obj5==obj4-8");
	void* obj6 = AlignedMemory_alloc(mm,6, MemoryKind_normal);
	Test_assert("Memory.re-alloc", obj6 == obj1, "应该在被释放的内存上obj6==obj1");
	AlignedMemoryReleaseInfo rs = AlignedMemory_collectGarbages(mm,1 ,0);
	Test_assert("Memory.collectGarbages", rs.bytes==0, "垃圾回收，所有页面都没有空闲，无法回收页面");
	AlignedMemory_free(mm, obj6);
	AlignedMemory_free(mm, obj2);
	rs = AlignedMemory_collectGarbages(mm, 1,0);
	Test_assert("Memory.collectGarbages", rs.bytes == opts.pageSize && rs.pages==1, "释放obj6,obj2,让page1页面空闲，回收一个页面");
...
void testAlignedLinkUnitMemory() {
	Test_begin("AlignedMemory(LINK unit,32bits)", "Test start...");
	AlignedMemoryOptions opts;
	opts.pageSize = 36;
	opts.totalBytes = 0;
	opts.gcBytes = 0;
	...
	AlignedMemory_free(mm, obj2);
	Test_assert("Memory.free" , obj2 == chunk->page->next->free, "归还内存obj2[%p]，page上放着空闲块链\n",obj2);
	...
	AlignedMemory_free(mm, obj3);
	Test_assert("Memory.free", obj3 == chunk->page->next->free && obj2==chunk->page->next->free->next, "归还内存obj3[%p]，page上放着空闲块链", obj3);
	AlignedMemory_free(mm, obj1);
	Test_assert("Memory.free", obj1 == chunk->page->next->free , "归还内存obj1[%p]，page上放着空闲块链", obj1);

	void* obj11 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj11 + 1);
	Test_assert("Memory.alloc", obj11 == obj4, "再次分配内存，使用空闲单元obj4==obj11[%p]", obj11);

	void* obj12 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj12 + 1);
	Test_assert("Memory.alloc", obj12 == (byte_t*)obj4 - 8, "分配内存obj12，占据page2[%p]", obj12);
	void* obj13 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj13 + 1);
	Test_assert("Memory.alloc", obj13 == (byte_t*)obj12 - 8, "分配内存obj13，占据page2[%p]", obj13);
	void* obj14 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	Test_assert("Memory.alloc", obj14 == obj1, "分配内存obj14，重复使用pag1上的空闲块obj14==obj1[%p]", obj14);
	...
	rs = AlignedMemory_collectGarbages(mm, 1,0);
	Test_assert("Memory.collectGarbages", rs.bytes == opts.pageSize && rs.pages==1, "释放obj14,让page1页面空闲，回收一个页面\r");
	Test_assert("Memory.collectGarbages", chunk->page && chunk->page->next==0 && chunk->page+1== obj13, "只有一个页面\r");

	bool_t sfreeRs = AlignedMemory_sfree(mm,obj13);
	Test_assert("Memory.sfree", sfreeRs && obj13==0, "安全释放宏，由于原先的内存放着空链指针，不可以再使用，释放后原先的指针应该不能被使用\t");
...
void testAlignedRefUnitMemory() {
	...
	AlignedMemory_free(mm, obj2);
	Test_assert("Memory.free", ((MemoryRefUnit*)obj2)->ref==0 , "归还内存obj2[%p]，引用计数置为0\n", obj2);
	AlignedMemory_free(mm, obj4);
	Test_assert("Memory.free"
		// 处在最后一个位置
	...
	void* obj11 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj11 + 1);
	Test_assert("Memory.alloc", obj11 == obj4, "再次分配内存obj11，使用空闲单元obj4==obj11[%p]", obj11);

	void* obj12 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj12 + 1);
	Test_assert("Memory.alloc", obj12 == (byte_t*)obj11+8, "分配内存obj12，占用obj11后面的空间[%p]", obj12);

	void* obj13 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj13 + 1);
	Test_assert("Memory.alloc", obj13 == (byte_t*)obj12 + 8, "分配内存obj13，占用obj12后面的空间[%p]", obj12);

	void* obj14 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj14 + 1);
	Test_assert("Memory.alloc", obj14 == obj2, "分配内存obj13，占用obj2原先的空间[%p]", obj12);

	AlignedMemory_free(mm, obj3);
	Test_assert("Memory.free", ((MemoryRefUnit*)obj3)->ref == 0, "归还内存obj3[%p]，引用计数置0", obj3);
	AlignedMemory_free(mm, obj1);
	Test_assert("Memory.free", ((MemoryRefUnit*)obj3)->ref == 0, "归还内存obj1[%p]，引用计数置0", obj1);

	void* obj15 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj15 + 1);
	Test_assert("Memory.alloc", obj15 == obj1, "分配内存obj15，obj15 == obj1[%p]", obj15);
	void* obj16 = AlignedMemory_alloc(mm, 5, MemoryKind_normal);
	ref_inc((MemoryRefUnit*)obj16 + 1);
	Test_assert("Memory.alloc", obj16 == obj3, "分配内存obj14，重复使用pag1上的空闲块obj16==obj3[%p]", obj16);
	...
	rs = AlignedMemory_collectGarbages(mm, 1, 0);
	Test_assert("Memory.collectGarbages", rs.bytes == opts.pageSize && rs.pages == 1, "释放obj14,让page1页面空闲，回收一个页面\r");
	Test_assert("Memory.collectGarbages", chunk->page && chunk->page->next == 0 && (&chunk->page->free == obj11), "只有一个页面\r");

	bool_t sfreeRs = AlignedMemory_sfree(mm, obj13);
	Test_assert("Memory.sfree", sfreeRs && obj13 == 0, "安全释放宏，由于原先的内存放着空链指针，不可以再使用，释放后原先的指针应该不能被使用\t");

	Test_end(0, "AlignedMemory(REF unit,32bits)", "Test done!");
	
}

void testAlignedMemory() {
	memories_module(0, 0, 0);
	Test_begin("AlignedMemory","");
