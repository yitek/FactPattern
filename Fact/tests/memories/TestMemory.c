...
void testMemory() {
	...
	int* obj4 = (int*)Memory_alloc(mm,4, MemoryKind_readwrite);
	*obj4 = 88;
	log_assert("Memory.require", obj4!=0,"请求4bytes内存obj4[%p]=%d\n", obj4,*obj4);

	long_t* obj8 = (long_t*)Memory_alloc__virtual__(mm,  8, MemoryKind_readwrite );
	*obj8 = 9876543210;
	...
	void* obj16 = Memory_alloc__virtual__(mm, 12,MemoryKind_writable);
...
void testAlignedMemory() {
	...
	void* obj1 = AlignedMemory_alloc(mm,6, MemoryKind_readwrite);
	...
	void* obj2 = AlignedMemory_alloc(mm,7, MemoryKind_readwrite);
	log_assert("Memory.alloc"
		// 在前面那个单元的前面
		, obj2==((byte_t*)obj1)-sizeof(addr_t)
		, "分配7个字节的单元[%p](total:%d)，会对齐到8个字节的单元,挨着第一个单元\n", obj2,mm->allocatedBytes);

	void* obj3 = AlignedMemory_alloc(mm,6, MemoryKind_readwrite);
	log_assert("Memory.alloc"
		// 翻页，有第二页
		, mm->chunks[0]->page->next!=0 
		// 第二页没有后继页面
	...
	void* obj4 = AlignedMemory_alloc(mm, 8, MemoryKind_readwrite);
	log_assert("Memory.re-alloc",obj4==obj3,"应该在被释放的内存上obj4==obj3");
	void* obj5 = AlignedMemory_alloc(mm, 8, MemoryKind_readwrite);
	log_assert("Memory.re-alloc", obj5 == obj4-8, "内存是连续的obj5==obj4-8");
	void* obj6 = AlignedMemory_alloc(mm,6, MemoryKind_readwrite);
...
void testAlignedMemory() {
	...
	void* obj3 = AlignedMemory_alloc(mm, 8, MemoryKind_readwrite);
	log_assert("Memory.alloc"
		// 在前面那个单元的前面
		, obj3 == ((byte_t*)obj2) - 8
	...
	void* obj4 = AlignedMemory_alloc(mm, 6, MemoryKind_readwrite);
	...
	void* obj11 = AlignedMemory_alloc(mm, 5, MemoryKind_readwrite);
	log_assert("Memory.alloc", obj11 == obj4, "再次分配内存，使用空闲单元obj4==obj11[%p]", obj11);

	void* obj12 = AlignedMemory_alloc(mm, 5, MemoryKind_readwrite);
	log_assert("Memory.alloc", obj12 == (byte_t*)obj4 - 8, "分配内存obj12，占据page2[%p]", obj12);
	void* obj13 = AlignedMemory_alloc(mm, 5, MemoryKind_readwrite);
	log_assert("Memory.alloc", obj13 == (byte_t*)obj12 - 8, "分配内存obj13，占据page2[%p]", obj13);
	void* obj14 = AlignedMemory_alloc(mm, 5, MemoryKind_readwrite);
