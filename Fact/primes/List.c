...
#include "List.h"
#define ALLOC(size,arg,kind,mm) m_alloc(size,arg,kind,mm)
#define ALLOC1(size,arg,kind,mm) m_alloc1(size,arg,kind,mm)
#define FREE(p,mm) m_free(p,mm)
...
List* List__construct__(List* self,TMemory* memory,void* listType,MemoryKinds mkind) {
	...
	if (!self) self = ALLOC(sizeof(List), listType,mkind, memory);
