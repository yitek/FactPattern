...
ObjectInstance* GCMemory_chunkCreatePage(MemoryChunk* chunk);
ObjectInstance* GCMemory_chunkRequire(GCMemory* self, MemoryChunk* chunk);
int GCMemory_chunkRelease(MemoryChunk* chunk, ObjectInstance* p, size_t pageSize);
void AllignAllocator_chunkSweep(MemoryChunk* chunk);
void AllignAllocator_chunkMark(MemoryChunk* chunk);

ObjectInstance* GCMemory_pageFindIdle(struct stMemoryPage* page, size_t capacity, size_t unitSize);

void AllignAllocator_markGCObject(ObjectInstance* gcObj);
...
ObjectInstance* GCMemory_chunkRequire(GCMemory* allocator, MemoryChunk* chunk) {
	size_t unitSize = chunk->unitSize, pageSize = chunk->pageSize;
	struct stMemoryPage* page = chunk->headPage;
	ObjectInstance* unit = GCMemory_pageFindIdle(page, chunk->pageCapacity, unitSize);
...
ObjectInstance* GCMemory_pageFindIdle(struct stMemoryPage* page, size_t capacity, size_t unitSize) {
	while (page) {
		ObjectInstance* unit = (ObjectInstance*)&page->first;
		size_t c = 0;
		while (c < capacity) {
			if (unit->ref == 0) {
				return unit;
			...
			c++;
			unit = (ObjectInstance*)((char*)unit + unitSize);
		}
		//没找到空闲单元，找next 页
		page = page->next;
	}
...
ObjectInstance* GCMemory_chunkCreatePage(MemoryChunk* chunk) {
	...
	for (size_t i = 0; i < chunk->pageCapacity; i++) {
		unit->ref = 0;
		unit = (ObjectInstance*)((char*)unit + chunk->unitSize);
	}
	if (chunk->tailPage) {
...
void AllignAllocator_chunkMark(MemoryChunk* chunk) {
	...
	MemoryPage* page = chunk->headPage;
	while (page) {
		ObjectInstance* gcObj = (ObjectInstance*)&page->first;
...
void AllignAllocator_markGCObject(ObjectInstance* gcObj) {
	...
	for (int i = 0, j = type->members->length; i < j; i++) {
		if (member->memberType->kind == TypeKind_Class) {
			...
			ObjectInstance* memberGCObj = GCObject_fromObject(memberObj);
